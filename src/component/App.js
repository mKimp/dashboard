...
class App extends React.Component {
  ...
  handleSubmit(e) {
    ...
    this.state.usstates.forEach((element) => {
      if (element === search) {
        //check if the user input the correct state name
      ...
      if (element === search) {
        const parks = this.props.data
          .filter(function (item) {
            //return back all the national parks info
            return item.states[0].title === search;
          })
          .map(function (item) {
            return item.title;
          });
        const parkSize = this.props.data
          .filter(function (item) {
            //return back the sizes of all national parks
            return item.states[0].title === search;
          })
          .map(function (item) {
        ...
        const visitors = this.props.data
          .filter(function (item) {
            // return back all the amount of visitors in the national parks
            return item.states[0].title === search;
          })
          .map(function (item) {
        ...
        const cood = this.props.data
          .filter(function (item) {
            //return back the coordinates of each national parks
            return item.states[0].title === search;
          })
          .map(function (item) {
    ...
    current.forEach((element) => {
      ...
      const newData = {
        //create the array of objects that contain coordinate, description, and established date of each national park for using in Map
        title: title,
  ...
  render() {
    ...
    if (!this.state.isLoaded && !this.state.hasResult) {
      ...
      !this.state.correctstateName
    ) {
      //the user input something but not one of the us states

      return (
        <AlertDismissible message={"You could misspell the state name?"} />
      );
    } else if (
      this.state.isLoaded &&
      this.state.correctstateName &&
      ...
      // the user input correct name of a state but that state does not have national parks
      return (
        <AlertDismissible
          message={"Sorry, this state DOES NOT have any National Parks"}
        />
      );
    } else {
      ...
      return (
        <Dashboard
          dataLabel={this.state.parkLabel}
          dataSize={this.state.parkSize}
